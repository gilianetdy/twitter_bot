...
def handle
	...
	handle_test = ["@EHachei", "@bakin_pancakes", "@Truccc5", "@Jahlawa", "@FralpsTHP"]
	selection = handle_test.sample(4)
	selection.each do |i|
		@client.update("vive les news du jour !  #bonjour_monde #{i} @the_hacking_pro ")
	end
end
...
def follow_hello
	@client.search('#bonjour_monde', result_type: 'recent').take(20).collect do |i|
